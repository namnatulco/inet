...
void PhysicalEnvironment::parseObjects(cXMLElement *xml)
{
    ...
    for (cXMLElementList::const_iterator it = children.begin(); it != children.end(); ++it)
    {
        ...
        if ((!_isNaN(spaceMin.x) && min.x < spaceMin.x) || (!_isNaN(spaceMax.x) && max.x > spaceMax.x) ||
            (!_isNaN(spaceMin.y) && min.y < spaceMin.y) || (!_isNaN(spaceMax.y) && max.y > spaceMax.y) ||
            (!_isNaN(spaceMin.z) && min.z < spaceMin.z) || (!_isNaN(spaceMax.z) && max.z > spaceMax.z))
            throw cRuntimeError("Object is outside of space limits");
        if (_isNaN(computedSpaceMin.x) || min.x < computedSpaceMin.x) computedSpaceMin.x = min.x;
        if (_isNaN(computedSpaceMin.y) || min.y < computedSpaceMin.y) computedSpaceMin.y = min.y;
        if (_isNaN(computedSpaceMin.z) || min.z < computedSpaceMin.z) computedSpaceMin.z = min.z;
        if (_isNaN(computedSpaceMax.x) || max.x > computedSpaceMax.x) computedSpaceMax.x = max.x;
        if (_isNaN(computedSpaceMax.y) || max.y > computedSpaceMax.y) computedSpaceMax.y = max.y;
        if (_isNaN(computedSpaceMax.z) || max.z > computedSpaceMax.z) computedSpaceMax.z = max.z;
    }
    if (_isNaN(spaceMin.x)) spaceMin.x = computedSpaceMin.x;
    if (_isNaN(spaceMin.y)) spaceMin.y = computedSpaceMin.y;
    if (_isNaN(spaceMin.z)) spaceMin.z = computedSpaceMin.z;
    if (_isNaN(spaceMax.x)) spaceMax.x = computedSpaceMax.x;
    if (_isNaN(spaceMax.y)) spaceMax.y = computedSpaceMax.y;
    if (_isNaN(spaceMax.z)) spaceMax.z = computedSpaceMax.z;
}
