...
void SimpleEnergyStorage::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (!_isNaN(nodeStartCapacity.get()) || !_isNaN(nodeShutdownCapacity.get())) {
...
{
    if (!_isNaN(nodeShutdownCapacity.get()) && newResidualCapacity <= nodeShutdownCapacity && nodeStatus->getState() == NodeStatus::UP) {
    ...
    else if (!_isNaN(nodeStartCapacity.get()) && newResidualCapacity >= nodeStartCapacity && nodeStatus->getState() == NodeStatus::DOWN) {
...
void SimpleEnergyStorage::scheduleTimer()
{
    ...
    if (totalPower > W(0)) {
        targetCapacity = _isNaN(printCapacityStep.get()) ? nominalCapacity : ceil(unit(residualCapacity / printCapacityStep).get()) * printCapacityStep;
        ...
        if (!_isNaN(nodeStartCapacity.get()) && nodeStatus->getState() == NodeStatus::DOWN && residualCapacity < nodeStartCapacity && nodeStartCapacity < targetCapacity)
            targetCapacity = nodeStartCapacity;
    }
    else if (totalPower < W(0)) {
        targetCapacity = _isNaN(printCapacityStep.get()) ? J(0) : floor(unit(residualCapacity / printCapacityStep).get()) * printCapacityStep;
        // make sure capacity will change over time despite double arithmetic
        simtime_t remainingTime = unit((targetCapacity - residualCapacity) / totalPower / s(1)).get();
        if (remainingTime == 0)
            targetCapacity -= printCapacityStep;
        // override target capacity if shutdown is needed
        if (!_isNaN(nodeShutdownCapacity.get()) && nodeStatus->getState() == NodeStatus::UP && residualCapacity > nodeShutdownCapacity && nodeShutdownCapacity > targetCapacity)
